...
if __name__ == '__main__':
    ...
    seq_len = X.size(2)
    logger.debug("seq_len {}".format(seq_len))
    assert seq_len > 1
    assert class_dim > 1
    ...
    generator = Generator(seq_len, args.latent_dim)
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, (traces, c) in enumerate(dataloader):
            ...
            fake_traces = generator(z)
            # Real traces
            real_validity = discriminator(real_traces)
            # Fake traces
            fake_validity = discriminator(fake_traces)
        ...
        if (epoch == 0) or (epoch + 1) % args.freq == 0:
            ...
            total_real = np.squeeze(total_real, axis=(1,))
            total_fake = np.squeeze(total_fake, axis=(1,))
            logger.info("total_real.shape:{}".format(total_real.shape))
